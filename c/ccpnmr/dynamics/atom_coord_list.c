...
Atom_coord_list new_atom_coord_list(int natoms, float *mass,
		float *x, float *y, float *z, int *isFixed)
{
    int i;
    ...
    for (i = 0; i < natoms; i++)
    {
	atom_coord = new_atom_coord(mass[i], x[i], y[i], z[i], isFixed[i]);
	if (!atom_coord)
	    return NULL;
...
CcpnStatus add_atom_coord_list(Atom_coord_list atom_coord_list,
		float mass, float x, float y, float z, int isFixed, CcpnString error_msg)
{
    int n;
