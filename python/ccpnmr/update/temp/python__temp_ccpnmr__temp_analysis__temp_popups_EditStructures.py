  ...
  **Model Superposition & RMSD Calculation**
  ...
  def importStructure(self, fileNames=None):
    ...
    if ensemble:
      ...
      # wb104 22 Sep 2015: hack because it looks like coordinates do not have callbacks
      # and this seems to be the only (or one) way to update the display correctly
      # but looks like also need to update pulldown menus since those are not updated properly
      popup = self.guiParent.popups.get('view_structure')
      if popup:
        popup.updateMolSystems()
        popup.updateStructures()
        popup.updateModels()
        popup.structFrame.displayStructure()
